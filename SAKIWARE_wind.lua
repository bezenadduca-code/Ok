...
-- Item TP (Auto Pickup)
------------------------------------------------------------------------
local secItemTP = tabGlobal:Section({ Title = "Item TP", Opened = true })
local itemTP = { on = false, tpSpeed = 100, thread = nil }

local function itemTPGetItems()
    local items = {}
    pcall(function()
...
local function itemTPRun()
    if itemTP.thread then return end
    itemTP.thread = task.spawn(function()
        pcall(function()
            ...
            -- save original position and speed
            local savedCF    = hrp.CFrame
            local savedSpeed = hum.WalkSpeed

            local items = itemTPGetItems()
            if #items == 0 then
                itemTP.thread = nil
                return
            end

            -- disable controls so player doesn't fight the teleport
            local controls = nil
            pcall(function()
            ...
            hum.WalkSpeed = itemTP.tpSpeed

            for _, item in ipairs(items) do
                pcall(function()
                    if not item.part or not item.part.Parent then return end
                    if espItemHeld(item.obj) then return end -- skip if someone grabbed it

                    -- TP to item
                    hrp.CFrame = item.part.CFrame + Vector3.new(0, 3, 0)
                    task.wait(0.1)

                    -- trigger ProximityPrompt if present
                    local prompt = item.obj:FindFirstChildOfClass("ProximityPrompt")
                        or item.obj:FindFirstChildWhichIsA("ProximityPrompt", true)
                    if prompt then
                        pcall(function()
...
secItemTP:Toggle({ Title = "Auto-Run on Round Start", Type = "Checkbox", Flag = "itemTPAuto", Default = false,
    Callback = function(on) itemTP.on = on end
})
secItemTP:Slider({ Title = "TP Speed", Flag = "itemTPSpeed", Step = 10,
    Value = { Min = 16, Max = 300, Default = 100 },
    Callback = function(v) itemTP.tpSpeed = v end
})
secItemTP:Button({ Title = "▶ Collect Items Now", Callback = function()
    if itemTP.thread then return end
    itemTPRun()
end })

-- auto-trigger when a round starts (Ingame folder appears)
pcall(function()
    svc.WS:WaitForChild("Map", 10)
    svc.WS.Map.ChildAdded:Connect(function(child)
        if child.Name == "Ingame" and itemTP.on then
            task.wait(2) -- let items spawn
            itemTPRun()
        end
    end)
end)
...
espAttach = function(obj, tag, color, isChar)
    ...
    local _pp; pcall(function() _pp = obj.PrimaryPart end)
    local root = obj:FindFirstChild("HumanoidRootPart") or _pp or obj:FindFirstChildWhichIsA("BasePart") or obj:FindFirstChild("Base") or obj:FindFirstChild("Main")
