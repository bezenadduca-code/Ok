...
------------------------------------------------------------------------
------------------------------------------------------------------------
-- TAB: ESP (Forsaken Document / Ring ESP)
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabESP = win:Tab({ Title = "ESP", Icon = "eye" })
local secESPForsaken = tabESP:Section({ Title = "Item ESP", Opened = true })

local _espEnabled       = false
local _currentESP       = nil
local _currentBillboard = nil

local function _removeESP()
    if _currentESP and _currentESP.Parent then _currentESP:Destroy() end
    if _currentBillboard and _currentBillboard.Parent then _currentBillboard:Destroy() end
    _currentESP = nil
    _currentBillboard = nil
end

local function _makeESP(obj)
    if _currentESP then return end
    ...
    pcall(function()
        ...
        local itemType = nil
        if txt:find("collect") or txt:find("document") or txt:find("folder") then
            itemType = "DOCUMENT"
        end
        ...
        _currentESP       = h
        _currentBillboard = bill
        print(itemType .. " FOUND:", obj:GetFullName())
    end)
end

local function _scanESP(v)
    pcall(function()
        if v:IsA("MeshPart") then _makeESP(v) end
    end)
end

secESPForsaken:Toggle({
...
svc.WS.DescendantAdded:Connect(function(v)
    task.wait(0.1)
    if _espEnabled and not _currentESP then _scanESP(v) end
end)
...
do
    ...
    function ChatLogger.createUI()
        ...
        pcall(function()
            ...
            titleText.BackgroundTransparency = 1; titleText.Text = "[chat] Chat Logger"
...
secMinion:Button({ Title="[rescan] Force Rescan", Callback=function() clearTag("pizza"); clearTag("zombie"); clearTag("puddle"); task.wait(0.1); scanPizza(); scanZombie(); scanPuddles() end })
...
secLMS:Button({ Title="v  Preload LMS", Callback=function() for name in pairs(musicTracks) do task.spawn(function()musicFetch(name)end); task.wait(0.1) end end })
